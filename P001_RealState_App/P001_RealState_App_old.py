...
def portfolio_density(data, geofile):
    ...
    exp_filters = st.beta_expander("Click here to see available filters", expanded=False)
    with exp_filters:
        # === creating filters
        ...
        st.write("**Zipcode:**")
        f_zipcode = st.multiselect('Type or select zipcodes', data['zipcode'].sort_values(ascending=True).unique())

        st.write("**Maximum Buying Price**:")
        p_min = int(data['buying_price'].min())
        p_max = int(data['buying_price'].max())
        f_price = st.slider('Select maximum price', p_min, p_max, step=1000)
        # if f_price:
        #     data = data[data['buying_price']<=f_price]
        # else:
        #     data = data.copy()

        st.write("**Distance from lake:**")
        d_min = int(data['dist_fromlake'].min())
        d_max = int(data['dist_fromlake'].max())
        f_distance = st.slider('Select distance from lake', d_min, d_max)
        # if f_distance:
        #     data = data[data['dist_fromlake']<=f_distance]
        # else:
        #     data = data.copy()
    ...
    with exp_density:
        ...
        with c1:
            ...
            if (f_decision is True) & (f_zipcode!=[]):
                f_data = data.loc[(data['decision'] == 1) & (data['zipcode'].isin(f_zipcode)), :]
            elif (f_decision is True) & (f_zipcode==[]):
                f_data = data.loc[data['decision'] == 1, :]
            elif (f_decision is False) & (f_zipcode!=[]):
                f_data = data.loc[(data['zipcode'].isin(f_zipcode)), :]
            else:
                f_data = data.copy()
        ...
        with c2:
            ...
            if (f_decision==True) & (f_zipcode!=[]):
                ...
                f_data = data.copy()
                st.write('*All properties available are been shown.*')
            st.dataframe(f_data[['id', 'zipcode', 'buying_price', 'expected_profit', 'decision']])
            # add id filter
